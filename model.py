...
class POPVAE(nn.Module):
    ...
    def __init__(self, x_h, x_w, h_dim, z_dim, u_dim, mb_size, nc,device,num_parts,num_parts_h,num_parts_w,part_h, part_w,t,M):
        ...
        self.u1_dim = u_dim # dimension of u', here ley u_dim=10, then for each part, the dim of u is 3
        ...
        #self.h2zmu = nn.Linear(h_dim,self.z_large_dim)
        #self.h2zvar = nn.Linear(h_dim,self.z_large_dim)
        self.tf = t

        
        
        #layer for local transformation of each part
        ...
        #self.u2u = nn.Linear(self.u_large_dim,self.u1_dim*self.num_parts)
        '''separate generate u
        '''
        self.u2u = nn.Linear(self.u1_dim,self.u1_dim)
        #self.u2u2 = nn.Linear(self.u1_dim,self.u2_dim)
        #self.u2rt = nn.Linear(self.u1_dim,3)
        #self.z2h = nn.Linear(z_dim, h_dim)
        
        #self.z2z = nn.Linear(self.z_large_dim,self.z_dim*self.num_parts)
        '''separate generate z
        '''
        #self.z2z1=nn.Linear(self.z_dim,self.z_dim)
        ...
        #    nn.Linear(self.h_dim, self.h_dim),
        #    nn.ReLU(),
        #    nn.Linear(self.h_dim, self.part_dim),
        #    nn.Sigmoid()
        #)
    ...
    def forward(self, inputs):
        ...
        '''
        #z_large_mu = self.h2zmu(h)
        #z_large_logvar = F.threshold(self.h2zvar(h), -6, -6)
        #z_large_var = torch.exp(z_large_logvar)
        '''
        ...
        #z_large = self.sample_z(z_large_mu, z_large_var).view(-1,self.num_parts,self.z_dim)
        u_large = self.sample_u(u_large_mu, u_large_var).view(-1,self.num_parts,self.u1_dim)
        
        #z = self.z2z1(z_large.view(-1,self.z_dim))
        ...
        '''
        #u = self.u2u(u_large.view(-1,self.u1_dim))
